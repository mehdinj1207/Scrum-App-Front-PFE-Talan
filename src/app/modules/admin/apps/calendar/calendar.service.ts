...
import { Meet } from '../../customdash/meet/meet.types';
...
export class CalendarService {
     ...
     * @param replace
     */
    getEvents(start: Moment, end: Moment, meets: Meet[], replace = false): Observable<CalendarEvent[]> {
                ...
                map((events) => {
                    ...
                    response.forEach((event, index) => {
                        if (index < meets.length) {
                            event.id=meets[index].idMeet.toString();
                            event.title = meets[index].title;
                            const dateStart = moment(meets[index].startDate);
                            event.start= dateStart.utc().format().toString().replace('Z', '.000Z');
                            const dateEnd = moment(meets[index].endDate);
                            event.end= dateEnd.utc().format().toString().replace('Z', '.000Z');
                            event.recurrence=null;
                            event.allDay=false;
                            event.description="http://localhost:4200/dashboard/meet/"+meets[index].roomName;
                            newResponse.push(event);
                        } else {
                            event.title="laal";
                    ...
                    if (meets.length > response.length) {
                        while(i<meets.length){
                            response.forEach((event, index) => {
                                if (index < meets.length && i<meets.length) {
                                    event.id=meets[i].idMeet.toString();
                                    event.title = meets[i].title;
                                    const dateStart = moment(meets[i].startDate);
                                    event.start= dateStart.utc().format().toString().replace('Z', '.000Z');
                                    const dateEnd = moment(meets[i].endDate);
                                    event.end= dateEnd.utc().format().toString().replace('Z', '.000Z');
                                    event.recurrence=null;
                                    event.allDay=meets[i].allDay;
                                    event.description=meets[i].description;
                                    newResponse.push(event);
                                    i++;
                                }
                            })
    ...
    prefetchFutureEvents(end: Moment): Observable<CalendarEvent[]> {
        ...
        // Prefetch the events
        let meets:Meet[]
        return this.getEvents(start, end, meets);
    }
