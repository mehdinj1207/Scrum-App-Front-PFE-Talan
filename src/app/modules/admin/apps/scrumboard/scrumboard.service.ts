...
import { Ticket, WorkflowItem } from '../../customdash/project/classes/sprint.types';

@Injectable({
    providedIn: 'root'
})
export class ScrumboardService
{   url = AppComponent.urlProjet + '/workflow';
    // Private
     ...
     * OUMAIMA CODE
     */
    getWorkflowsBySprint(idSprint:number){
        return this._httpClient.get<WorkflowItem[]>(this.url+"/sprint/"+idSprint);
    }

    // MedZied
    UpdateTicketWorkflowItem(idTicket:number,idWorkflowItem:number):Observable<Ticket>{
        return this._httpClient.get<Ticket>("http://localhost:8090/tickets/"+idTicket+"/workflowItem/"+idWorkflowItem);
    }

    addNewWorkflowItem(idSprint:number,workflowItem:WorkflowItem):Observable<WorkflowItem>{
        return this._httpClient.post<WorkflowItem>("http://localhost:8090/workflow/sprint/"+idSprint,workflowItem);
    }

    deleteWorkflowItem(idWorkflowItem:number,newIdWorkflowItem:number):Observable<Object>{
        
        return this._httpClient.delete("http://localhost:8090/workflow/"+idWorkflowItem+"/new-workflow/"+newIdWorkflowItem)
    }

    //Update the name of wfItem
    updateWorkflowItem(workflowItem:WorkflowItem,idWorkflowItem:number):Observable<Object>{
        return this._httpClient.put("http://localhost:8090/workflow/"+idWorkflowItem,workflowItem);
    }

    //Update the order of the workflow
    updateWorkflowItemsOrder(updatedWorkflowItems:WorkflowItem[]):Observable<any>{
        return this._httpClient.put("http://localhost:8090/workflowOrder",updatedWorkflowItems);
    }
