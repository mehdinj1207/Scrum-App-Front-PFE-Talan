...
import { ProjectContentComponent } from './modules/admin/customdash/project/ressources-sprints/project-content/project-content.component'; 
//Import Keycloak AuthGuard
import { AuthGuard } from './utility/app.guard';
import { SprintAddComponent } from './modules/admin/customdash/project/ressources-sprints/crud-sprint/add-sprint/add-sprint.component';
import { CanDeactivateAddSprint } from './modules/admin/customdash/project/project.guards';
...
export const appRoutes: Route[] = [
    ...
    // Redirect empty path to 'customdash/users'
    { path: '', pathMatch: 'full', redirectTo: 'dashboard/users' },

    // Redirect signed in user to the 'customdash/users'
    //
    // After the user signs in, the sign in page will redirect the user to the 'signed-in-redirect'
    // path. Below is another redirection for that path to redirect the user to the desired
    // location. This is a small convenience to keep all main routes together here on this file.
    { path: 'signed-in-redirect', pathMatch: 'full', redirectTo: 'customdash/users' },
    ...
    // Admin routes
    {
        ...
        children: [
            ...
            import('app/modules/admin/pages/settings/settings.module').then(m => m.SettingsModule) },

            //Custom Dashboard
            {
                path: 'dashboard', children: [
                    { path: 'users', loadChildren: () => 
                    import('app/modules/admin/customdash/user/user.module').then(m => m.UserModule) },

                    { path: 'projects', loadChildren: () => 
                    import('app/modules/admin/customdash/project/project.module').then(m => m.ProjectModule) },
                    { path: 'chat', loadChildren: () => 
                    import('app/modules/admin/apps/chat/chat.module').then(m => m.ChatModule) },

                    { path: 'meet/:chatRoomName', loadChildren: () => 
                    import('app/modules/admin/customdash/meet/meet.module').then(m => m.MeetModule) },
                    { path: 'analytics', loadChildren: () => 
                    import('app/modules/admin/dashboards/analytics/analytics.module').then(m => m.AnalyticsModule) },
                    
                    { path: 'calendar', loadChildren: () => 
                    import('app/modules/admin/apps/calendar/calendar.module').then(m => m.CalendarModule) },
                    { path: 'feedback', loadChildren: () => 
                    import('app/modules/admin/apps/mailbox/mailbox.module').then(m => m.MailboxModule) },
                    
                    { path: 'file-manager', loadChildren: () => 
                    import('app/modules/admin/apps/file-manager/file-manager.module').then(m => m.FileManagerModule) },
                    { path: 'scrumboard', loadChildren: () => import('app/modules/admin/apps/scrumboard/scrumboard.module').then(m => m.ScrumboardModule) },
                ]
            },
            // Dashboards
            {
                path: 'dashboardsFuse', children: [
